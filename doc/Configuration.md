# Configuration Guide <br/> Activities Microservice

Activities microservice configuration structure follows the 
...
Example to retrieve microservice configuration remote repository by key 'pip-services-activities':
```javascript
{
    "config": {
        "type": "remote",
        ...
        "key": "pip-services-activities" // Microservice config key
    } 
    ...
}
```
...
- path: string - file path where PartyActivity objects are stored. The object are written into the file in JSON format.
- data: [PartyActivity] - (optional) array of PartyActivity objects which is used to initialize the dataset. This option is only used in testing to set a known state on test start.
...
However, Activities microservice doesn't have any dependencies. So, this section should be omitted entirely or kept empty.
