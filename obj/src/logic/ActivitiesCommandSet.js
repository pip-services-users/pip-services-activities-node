...
class ActivitiesCommandSet extends pip_services_commons_node_1.CommandSet {
    ...
    makeGetPartyActivitiesCommand() {
        return new pip_services_commons_node_2.Command("get_party_activities", new pip_services_commons_node_5.ObjectSchema(true)
    ...
    makeLogPartyActivityCommand() {
        return new pip_services_commons_node_2.Command("log_party_activity", new pip_services_commons_node_5.ObjectSchema(true)
            ...
            activity.time = pip_services_commons_node_9.DateTimeConverter.toNullableDateTime(activity.time);
            this._logic.logPartyActivity(correlationId, activity, callback);
        });
    }
    makeBatchPartyActivitiesCommand() {
        return new pip_services_commons_node_2.Command("batch_party_activities", new pip_services_commons_node_5.ObjectSchema(true)
            .withRequiredProperty('activities', new pip_services_commons_node_6.ArraySchema(new PartyActivityV1Schema_1.PartyActivityV1Schema())), (correlationId, args, callback) => {
            let activities = args.getAsArray("activities");
            _.each(activities, (a) => {
                a.time = pip_services_commons_node_9.DateTimeConverter.toNullableDateTime(a.time);
            });
    ...
    makeDeletePartyActivitiesCommand() {
        return new pip_services_commons_node_2.Command("delete_party_activities", new pip_services_commons_node_5.ObjectSchema(true)
